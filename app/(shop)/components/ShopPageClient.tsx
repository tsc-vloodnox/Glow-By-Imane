...
export function ShopPageClient({ products, categories }: ShopPageClientProps) {
  ...
  const groupedByCategory = useMemo(() => {
    ...
    return categories
      .map((category) => ({
        category,
        items: filteredProducts.filter((product) => product.category.id === category.id),
      }))
      ...
      >
        <div className="relative h-[220px] rounded-3xl overflow-hidden bg-[var(--color-sand)] flex items-end">
          ...
          <div
            ...
            style={{ background: "linear-gradient(135deg, #8B1A3A 0%, #C4637B 40%, #EDE3DC 100%)" }}
          />
          {/* Logo de la boutique — remplace le src par ton fichier réel */}
          <img
            src="/logo-glow-by-imane.png"
            alt="Glow by Imane"
            className="absolute top-5 left-5 h-10 w-auto"
          />
            ...
            {groupedByCategory.map((group) => (
              <div key={group.category.id}>
                <div className="mb-3 flex items-baseline justify-between border-b border-[var(--color-border)] pb-2">
                  <h2 className="font-serif text-xl text-[var(--color-accent)]">{group.category.label}</h2>
                  <span className="text-[11px] text-[var(--color-muted)]">{group.items.length} article(s)</span>
                </div>
